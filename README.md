...
A full-stack application that leverages a **Multi-Agent AI Architecture** powered by the Google Gemini API to automatically generate professional documentation for any given code function. It features real-time Server-Sent Events (SSE) streaming and Abstract Syntax Tree (AST) validation to guarantee code logic integrity. This project is fully containerized with Docker and features a complete CI/CD pipeline for automated builds and cloud deployments.
...
* **Multi-Agent Orchestration:** A sophisticated 3-step pipeline (Context Architect, Documentation Agent, Verification Agent) ensures high-quality and structurally sound AI responses.
* **Real-Time Pipeline Tracking:** Uses Server-Sent Events (SSE) to stream live updates to the frontend UI stepper as each AI agent completes its task.
* **Cryptographic Logic Verification:** Employs AST extraction and hashing to mathematically guarantee the AI did not hallucinate or alter the structural logic of your original code.
...
This application moved beyond a simple wrapper into an advanced agentic pipeline designed for precision and structural integrity.

1. **Agent 1: Context Architect**
   * Parses your raw code string into an **Abstract Syntax Tree (AST)**.
   * Extracts constraints (variables used, structure) and generates a cryptographic **Logic Hash**.
2. **Agent 2: Documentation Agent**
   * Utilizes the constrained prompt from Agent 1 and passes it to the **Gemini 3 Flash LLM**.
   * Generates highly-accurate, markdown-formatted technical documentation.
3. **Agent 3: Verification Gatekeeper**
   * Takes the final markdown generated by Agent 2 and extracts any raw code blocks.
   * Parses the *new* code blocks directly back into an AST and compares it against the original **Logic Hash**.
   * If the hashes match, the UI gives a green `[VERIFIED LOGIC]` badge; otherwise, it throws a warning.
...
| **AI Integration** | `Google Gemini API`, `AST Parsing`, `Cryptographic Hashing`                           |
